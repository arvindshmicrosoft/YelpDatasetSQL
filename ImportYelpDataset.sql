...
select Business.business_id, category.value as category
INTO BusinessCategory
from Business
CROSS APPLY OPENJSON (categories) as category
GO

drop table if exists YelpUserFriend
GO

select user_id, userfriend.value as friend_user_id
into YelpUserFriend
from YelpUser
CROSS APPLY OPENJSON(friends) as userfriend
go
